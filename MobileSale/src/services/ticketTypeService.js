...
export default {
  ...
  async getGroundChangCisDtosVariedAsync(ticketTypeId, date){
    let response = await ajax.get(`/ticketType/GetGroundChangCisDtosVariedAsync?ticketTypeId=${ticketTypeId}&date=${date}`);
    return response.result;
  }
};
