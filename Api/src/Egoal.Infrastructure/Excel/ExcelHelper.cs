...
namespace Egoal.Excel
{
    public static class ExcelHelper
    {
        ...
        public static Task<byte[]> ExportToExcelAsync<T>(IEnumerable<T> data, string title, string memo, Dictionary<string, Func<string>> displayProvider = null)
        {
            return Task.Run(() =>
            {
                if (data == null)
                ...
                using (var package = new ExcelPackage())
                {
                    ...
                    foreach (var property in properties)
                    {
                        ...
                        var dynamicDisplayAttributes = property.GetCustomAttributes(typeof(DynamicDisplayAttribute), false);
                        var displayAttributes = property.GetCustomAttributes(typeof(DisplayAttribute), false);
                        if (!dynamicDisplayAttributes.IsNullOrEmpty() && !displayProvider.IsNullOrEmpty())
                        {
                            displayName = displayProvider[property.Name]();
                        }
                        else if (!displayAttributes.IsNullOrEmpty())
                        {
                        ...
                        worksheet.Cells[2, columnIndex].Value = displayName;
