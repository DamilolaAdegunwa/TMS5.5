...
";
        ...
        public async Task<DataTable> StatByCustomerAsync(StatPayDetailByCustomerInput input, IEnumerable<ComboboxItemDto<int>> payTypes)
        {
            StringBuilder whereBuilder = new StringBuilder();
            whereBuilder.AppendWhere("a.CTime>=@StartCTime");
            whereBuilder.AppendWhere("a.CTime<@EndCTime");
            whereBuilder.AppendWhere("b.CustomerID IS NOT NULL");
            whereBuilder.AppendWhereIf(input.CustomerId.HasValue, "b.CustomerID=@CustomerId");
            whereBuilder.AppendWhereIf(input.TradeSource.HasValue, "b.TradeSource=@TradeSource");
            whereBuilder.AppendWhereIf(input.PayTypeId.HasValue, "a.PayTypeID=@PayTypeId");
...
(
	...
	{whereBuilder}
	GROUP BY b.CustomerID,a.PayTypeID
)x
PIVOT(SUM(x.PayMoney) FOR x.PayTypeID IN ({payTypeColumns.ToString().TrimEnd(',')})) AS p
";
            var reader = await Connection.ExecuteReaderAsync(sql, input, Transaction);
