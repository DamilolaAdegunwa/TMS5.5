...
namespace Egoal.Tickets
{
    public class TicketConsumeRepository : EfCoreRepositoryBase<TicketConsume, long>, ITicketConsumeRepository
    {
        ...
        public async Task<PagedResultDto<TicketConsumeListDto>> QueryTicketConsumesAsync(QueryTicketConsumeInput input)
        {
            ...
            where.AppendWhereIf(input.StartTravelDate.HasValue, "b.SDate>=@StartTravelDate");
            where.AppendWhereIf(input.EndTravelDate.HasValue, "b.SDate<=@EndTravelDate");
            ...
            if (!string.IsNullOrEmpty(input.CustomerId))
            {
                ...
                if (Guid.TryParse(input.CustomerId, out customerId))
                {
                    where.AppendWhere($"b.CustomerID='{customerId}'");
                }
                else
                {
                    where.AppendWhere("b.CustomerId IS NULL");
                }
            }
...
a.ConsumeTime,
a.ConsumeTime AS LastNoticeTime,
...
a.TicketTypeId,
a.Price,
SUM(a.ConsumeNum) AS ConsumeNum,
SUM(CASE WHEN a.HasNoticed=1 THEN a.ConsumeNum ELSE 0 END) AS CheckNum
FROM dbo.TM_TicketConsume a WITH(NOLOCK)
LEFT JOIN dbo.TM_TicketSale b WITH(NOLOCK) ON b.ID=a.TicketId
{where}
GROUP BY b.CustomerID,b.CustomerName,a.TicketTypeId,a.Price
