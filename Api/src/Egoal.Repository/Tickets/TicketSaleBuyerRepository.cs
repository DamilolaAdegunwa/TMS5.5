...
";
        ...
        public async Task<IEnumerable<string>> GetAgeRangesAsync()
        {
        ...
        public async Task<DataTable> StatTouristByAgeRangeSimpleAsync(StatTouristByAgeRangeInput input)
        {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.AppendWhereIf(input.StartCTime.HasValue, "a.Ctime >= @StartCTime");
            stringBuilder.AppendWhereIf(input.EndCTime.HasValue, "a.Ctime <= @EndCTime");
            string sql = $@"
SELECT
x.AgeRange,
Count(x.Age)
...
(
	SELECT 
	a.Age,
	(CASE WHEN a.Age < 20 THEN '20岁以下' WHEN a.Age > 20 AND a.Age <= 40 THEN '20-40岁' WHEN a.Age > 40 AND a.Age < 60 THEN '40-60岁' WHEN a.Age > 60 THEN '60岁以上' ELSE '未知' END) AS AgeRange
	FROM TM_TicketSaleBuyer a
    {stringBuilder}
) x
GROUP BY x.AgeRange";

            IDataReader dataReader = await Connection.ExecuteReaderAsync(sql, input, Transaction);
...
";
            return (await Connection.QueryAsync<StatTouristListDto>(sql, new { input.StartCTime, input.EndCTime }, Transaction, TimeoutSeconds)).ToList();
        }
    }
}
