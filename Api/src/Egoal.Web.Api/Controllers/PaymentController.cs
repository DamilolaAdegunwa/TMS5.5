...
namespace Egoal.Web.Api.Controllers
{
    public class PaymentController : TmsControllerBase
    {
        ...
        [HttpPost]
        public async Task<string> JsApiPayAsync(string listNo)
        {
            return await _payAppService.OffiaccountPayAsync(listNo);
        }
        ...
        {
            return await _payAppService.MiniprogramPayAsync(listNo);
        }
        ...
        [UnitOfWork(IsDisabled = true)]
        [DontWrapResult]
        public async Task<string> ABCPayNotify()
        {
            string args = await Request.Body.ReadAsStringAsync();

            var output = await _payAppService.HandlePayNotifyAsync(args, DefaultPayType.农行支付);
