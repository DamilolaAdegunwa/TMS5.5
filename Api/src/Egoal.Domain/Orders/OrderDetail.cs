...
namespace Egoal.Orders
{
    public class OrderDetail : AuditedEntity<long>
    {
        ...
        public void AddOrderGroundChangCi(OrderGroundChangCi orderGroundChangCi)
        {
            if (OrderGroundChangCis == null)
            {
                OrderGroundChangCis = new List<OrderGroundChangCi>();
            }

            OrderGroundChangCis.Add(orderGroundChangCi);
        }

        public void AddOrderTourist(OrderTourist orderTourist)
        {
            if (OrderTourists == null)
            {
                OrderTourists = new List<OrderTourist>();
            }
        ...
        {
            TicketType = ticketType ?? throw new TmsException($"票类不存在");
            TicketTypeId = ticketType.Id;
            TicketTypeName = ticketType.GetDisplayName();
        ...
        {
            if (!ticPrice.HasValue)
            {
                throw new UserFriendlyException($"{TicketType.GetDisplayName()}票价未设置");
            }
        ...
        {
            if (!reaPrice.HasValue)
            {
                throw new UserFriendlyException($"{TicketType.GetDisplayName()}票价未设置");
            }

            ReaPrice = reaPrice;
            ReaMoney = totalNum * reaPrice.Value;
        }
