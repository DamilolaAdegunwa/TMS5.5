...
namespace Egoal.Orders
{
    public class Order : AuditedEntity<string>
    {
        ...
        public void Audit()
        {
            OrderStatusId = OrderStatus.已审核;
            OrderStatusName = OrderStatus.已审核.ToString();
        }

        public bool ShouldPay()
        {
            return !IsFree() && PayFlag != true && OrderStatusId == OrderStatus.已审核;
        }
        ...
        public void Cancel()
        {
            ...
            OrderStatusId = OrderStatus.已取消;
            OrderStatusName = OrderStatus.已取消.ToString();
            if (HasPaid())
            {
        ...
        public string GetProductInfo()
        {
            ...
            foreach (var orderDetail in OrderDetails)
            {
                builder.Append(orderDetail.TicketType?.GetDisplayName() ?? orderDetail.TicketTypeName).Append(orderDetail.TotalNum).Append("张,");
            }
