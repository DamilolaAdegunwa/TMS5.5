...
namespace Egoal.WeChat.Message
{
    public class MessageService : ITransientDependency
    {
        ...
        /// <returns></returns>
        public async Task<string> SendPaySuccessMessageAsync(string openId, string listNo, string totalMoney, string productInfo, string url)
        {
            string msgId = "";
            ...
            if (!string.IsNullOrEmpty(input.template_id))
            {
                input.url = url;
