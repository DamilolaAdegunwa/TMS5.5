            ...
            foreach (var item in eventData.Items)
            {
                ...
                if (ticketSale.OrderDetailId.HasValue)
                {
                    ...
                    foreach (var groundChangCi in groundChangCis)
                    {
                        ...
                        var quantity = ticketSale.CheckTypeId == CheckType.家庭套票 ? item.RefundQuantity * ticketSale.GetCheckNum() : item.RefundQuantity;
                        cancelGroundChangCiInput.Quantity = quantity;
