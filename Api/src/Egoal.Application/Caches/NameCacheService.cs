...
namespace Egoal.Caches
{
    public class NameCacheService : INameCacheService
    {
        ...
        public string GetPromoterName(int? id)
        {
            if (!id.HasValue)
            {
                return null;
            ...
            return _memoryCache.GetOrCreate($"PromoterName:{id}", entry =>
            {
                entry.SlidingExpiration = _slidingExpiration;

                var repository = _serviceProvider.GetRequiredService<IRepository<Promoter>>();
        ...
        public string GetParkNameByGround(int? groundId)
        {
            ...
            return _memoryCache.GetOrCreate($"ParkNameByGround:{groundId}", entry =>
            {
                entry.SlidingExpiration = _slidingExpiration;

                var repository = _serviceProvider.GetRequiredService<IRepository<Ground>>();

                var parkId = repository.GetAll().Where(o => o.Id == groundId).Select(o => o.ParkId).FirstOrDefault();

                return GetParkName(parkId);
            });
        }

        public string GetParkNameByTicketType(int? ticketTypeId)
        {
            ...
            return _memoryCache.GetOrCreate($"ParkNameByTicketType:{ticketTypeId}", entry =>
            {
                entry.SlidingExpiration = _slidingExpiration;

                var repository = _serviceProvider.GetRequiredService<ITicketTypeRepository>();

                var parkId = repository.GetAll().Where(o => o.Id == ticketTypeId).Select(o => o.ParkId).FirstOrDefault();
        ...
        public string GetStadiumName(int? id)
        {
            ...
            return _memoryCache.GetOrCreate($"StadiumName:{id}", entry =>
            {
                entry.SlidingExpiration = _slidingExpiration;

                var repository = _serviceProvider.GetRequiredService<IRepository<Stadium>>();
        ...
        public string GetRegionName(int? id)
        {
            ...
            return _memoryCache.GetOrCreate($"RegionName:{id}", entry =>
            {
                entry.SlidingExpiration = _slidingExpiration;

                var repository = _serviceProvider.GetRequiredService<IRepository<Region>>();

                return repository.GetAll().Where(o => o.Id == id).Select(o => o.Name).FirstOrDefault();
            });
        }

        public string GetCzkCztcName(int? id)
        {
            if (!id.HasValue)
            {
                return null;
            ...
            return _memoryCache.GetOrCreate($"CzkCztcName:{id}", entry =>
            {
                entry.SlidingExpiration = _slidingExpiration;

                var repository = _serviceProvider.GetRequiredService<IRepository<CzkCztc>>();
