    ...
    public class TradeAppService : ApplicationService, ITradeAppService
    {
        ...
        public async Task<PagedResultDto<TradeListDto>> QueryTradesAsync(QueryTradeInput input)
        {
            ...
            foreach (var trade in result.Items)
            {
                ...
                trade.PromoterName = _nameCacheService.GetPromoterName(trade.PromoterId);
            }
            ...
            else
            {
                ...
                foreach (DataRow row in table.Rows)
                {
                    ...
                    if (int.TryParse(row[statTypeColumn].ToString(), out int id))
                    {
                        ...
                        {
                            dataName = _nameCacheService.GetParkName(id);
                        }
                        if (input.StatType == PayDetailStatType.按售票点)
                        {
                            dataName = _nameCacheService.GetSalePointName(id);
                        }
                        if (input.StatType == PayDetailStatType.按收银员)
                        {
                            dataName = _nameCacheService.GetStaffName(id);
                        }
                    }
        ...
        public async Task<byte[]> StatPayDetailByCustomerToExcelAsync(StatPayDetailByCustomerInput input)
        {
            var result = await StatPayDetailByCustomerAsync(input);

            return await ExcelHelper.ExportToExcelAsync(result.Data, "客户收款汇总", string.Empty);
        }

        public async Task<DynamicColumnResultDto> StatPayDetailByCustomerAsync(StatPayDetailByCustomerInput input)
        {
            var payTypes = await _payTypeAppService.GetPayTypeComboboxItemsAsync();

            var table = await _payDetailRepository.StatByCustomerAsync(input, payTypes);
            foreach (DataColumn column in table.Columns)
            {
                if (int.TryParse(column.ColumnName, out int payTypeId))
                {
            ...
            DataColumn dataColumn = new DataColumn("客户名称");
            table.Columns.Add(dataColumn);
            foreach (DataRow row in table.Rows)
            {
                if (Guid.TryParse(row["CustomerID"].ToString(), out Guid customerId))
                {
                    row[dataColumn] = _nameCacheService.GetCustomerName(customerId);
                }
            }
