    ...
    public class BigDataAppService : ApplicationService, IBigDataAppService
    {
        ...
        public async Task<ResponseDto> ProcessRequestAsync(RequestDto request)
        {
            ...
            switch (request.method.ToLower())
            {
                ...
                case "stat_flow":
                    {
                        responseBody = await StatTicketCheckAsync(request.request_body);
                        break;
                    }
                case "query_consume":
                    {
        ...
        private async Task<QueryTicketSaleOutput> QueryTicketSaleAsync(string requestBody)
        {
            ...
            foreach (var item in items)
            {
                ...
                item.product_name = _nameCacheService.GetTicketTypeName(item.TicketTypeId);
                item.park_name = _nameCacheService.GetParkNameByTicketType(item.TicketTypeId);
        ...
        private async Task<List<StatTicketCheckListDto>> StatTicketCheckAsync(string requestBody)
        {
            var input = GetAndCheckInput<StatFlowInput>(requestBody);

            var statInput = new Tickets.Dto.StatTicketCheckInInput();
            statInput.StartCTime = input.start_date.Date;
            statInput.EndCTime = input.start_date.Date.AddDays(1);

            var table = await _ticketCheckRepository.StatTicketCheckByGroundAndTimeAsync(statInput);
            if (table.IsNullOrEmpty())
            {
                throw new TmsException("无数据");
            }
        ...
        private async Task<QueryTicketCheckOutput> QueryTicketCheckAsync(string requestBody)
        {
            ...
            foreach (var item in items)
            {
                ...
                item.park_name = _nameCacheService.GetParkNameByGround(item.GroundId);
            }
