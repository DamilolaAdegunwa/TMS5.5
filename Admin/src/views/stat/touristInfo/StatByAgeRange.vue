    ...
    <div class="search-box">
      <el-form ref="searchForm" :model="input" label-width="90px">
        <el-row type="flex">
          ...
          <el-col :md="6" :lg="12" :xl="12" class="button-box">
            <el-form-item label-width="20px">
...
export default {
  ...
  data() {
    return {
      ...
      input: new StatInput(),
      statTypes: [
        { value: 1, displayText: "按月" },
        { value: 2, displayText: "按年" }
      ],
      chartData: {
        columns: [],
        rows: []
  ...
  methods: {
    async getData() {
      ...
      await this.submit(async () => {
        const result = await ticketService.statTouristByAgeRangeAsync(
          this.input
        );
    ...
    async exportToExcel() {
      ...
      await this.submit(async () => {
        await ticketService.statTouristByAgeRangeToExcelAsync(this.input);
      });
    },
    checkInput() {
