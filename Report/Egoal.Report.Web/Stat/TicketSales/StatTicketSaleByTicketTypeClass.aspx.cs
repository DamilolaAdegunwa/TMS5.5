...
{
    public partial class StatTicketSaleByTicketTypeClass : PageBase
    {
        ...
        protected async void Page_Load(object sender, EventArgs e)
        {
            try
            {
                var queryInput = Request.QueryString.ToObject<StatTicketSaleByTicketTypeClassInput>();
                if (!int.TryParse(Request["height"], out int height))
                {
                    height = 500;
                }

                data = await ticketSaleAppService.StatTicketSaleByTicketTypeClassAsync(queryInput, Request["token"]);
                ...
                var pageReport = new PageReport(ActiveReportsHelper.GetReport("Tickets.StatTicketSaleByTicketTypeClass.rdlx"));
                var queryString = new StringBuilder($"统计区间:{queryInput.StartCTime}至{queryInput.EndCTime}");
                var ticketTypeClassName = Request["TicketTypeClassName"];
                if (!string.IsNullOrEmpty(ticketTypeClassName))
                {
                    queryString.Append($",票种:{ticketTypeClassName}");
                }
                ...
                if (isExport)
                {
                    await ExportToExcelAsync(pageReport.Document, "售票按票种统计");
                }
                else
                {
